...
export default function FlightPage() {
  ...
  useEffect(() => {
    ...
    map.current = L.map(mapContainer.current, { attributionControl: false })
  ...
  useEffect(() => {
    ...
    if (flight.departureAirport && target?.latitude) {
      plannedRef.current = L.polyline(
        [[[flight.departureAirport.latitude, flight.departureAirport.longitude], [target.latitude, target.longitude]]],
    ...
    if (flight.positions.length >= 2) heading = getHeading(flight.positions[flight.positions.length - 2], lastPos);
    ...
    if (lastPos) map.current.setView([lastPos.latitude, lastPos.longitude], 12);
    ...
    <main className="h-screen w-screen flex flex-col">
      <div className="bg-white shadow-md p-4 flex-shrink-0">
        ...
        <div className="flex items-center gap-6 flex-wrap mt-2">
          ...
          <div className="ml-auto flex gap-6 text-sm">
            ...
            <div>
              ...
              <span className={`font-medium ${flight.status === "active" ? "text-green-600" : flight.status === "scheduled" ? "text-blue-600" : "text-gray-600"}`}>
                {flight.status === "active" ? "В воздухе" : flight.status === "scheduled" ? "По расписанию" : flight.status === "completed" ? "Завершён" : flight.status}
              </span>
            </div>
        ...
        <button onClick={() => setShowDetails(!showDetails)}
          ...
          {showDetails ? "▲ Скрыть детали" : "▼ Подробнее о рейсе"}
        </button>
        ...
        {showDetails && (
          <div className="mt-3 bg-gray-50 rounded-lg p-4 text-sm space-y-2">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <span className="text-gray-400">Вылет по плану:</span><br />
                <span className="font-medium">{formatDate(flight.scheduledDeparture)}</span>
              </div>
              <div>
                <span className="text-gray-400">Прилёт по плану:</span><br />
                <span className="font-medium">{formatDate(flight.scheduledArrival)}</span>
              </div>
              <div>
                <span className="text-gray-400">Вылет фактический:</span><br />
                <span className="font-medium">{formatDate(flight.actualDeparture)}</span>
              </div>
              <div>
                <span className="text-gray-400">Прилёт фактический:</span><br />
                <span className="font-medium">{formatDate(flight.actualArrival)}</span>
              </div>
            </div>
          </div>
        )}
